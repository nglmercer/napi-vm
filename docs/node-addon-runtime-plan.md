...
which is itself an array and inherits from `Object.prototype`. Queries for
values whose built-in prototype is not represented (including proxies) return
a generic Node-API failure.
